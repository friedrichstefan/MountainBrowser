...
class TVOSWebViewHostController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        // FIX: Auf StopAllTimers reagieren (App geht in Hintergrund)
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(stopAllTimersNotification),
            name: NSNotification.Name("StopAllTimers"),
            object: nil
        )
    }
    
    // FIX: Timer stoppen wenn Notification kommt
    @objc private func stopAllTimersNotification() {
    ...
    // FIX: Timer stoppen wenn View verschwindet
    ...
    @objc func webViewDidFinishLoad(_ webView: UIView) {
        ...
        let extractAndNotify = { [weak self] in
            guard let self = self else { return }
    ...
    @objc func webView(_ webView: UIView, didFailLoadWithError error: Error) {
        ...
        if Thread.isMainThread {
            ...
            DispatchQueue.main.async { [weak self] in
                self?.coordinator?.webViewDidFail(error: error)
            }
        }
    }
    ...
    deinit {
        // FIX: Alle Timer und Notifications aufräumen
        moveTimer?.invalidate()
        moveTimer = nil
...
class TVOSWebViewController: ObservableObject {
    ...
    // FIX: Observer-Referenz für sauberes Aufräumen
    private var momentumScrollObserver: NSObjectProtocol?
    private var stopTimersObserver: NSObjectProtocol?
    
    init() {
        // FIX: Verwende block-basierte Observer für bessere Speicherverwaltung
        momentumScrollObserver = NotificationCenter.default.addObserver(
            forName: NSNotification.Name("StartMomentumScroll"),
            object: nil,
            queue: .main
        ) { [weak self] notification in
            guard let self = self,
                  let velocityY = notification.userInfo?["velocityY"] as? CGFloat else { return }
            self.startMomentumScroll(initialVelocity: velocityY)
        }
    ...
    deinit {
        // FIX: Sauberes Aufräumen aller Observer und DisplayLink
        if let observer = momentumScrollObserver {
